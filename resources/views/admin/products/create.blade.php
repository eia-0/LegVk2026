...
@section('admin-content')
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-2xl shadow space-y-4">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="preparation_time" value="{{ old('preparation_time', 0) }}" class="w-full border rounded p-2">
            </div>

            {{-- Вес --}}
            <div>
                <label class="block mb-1 font-medium">Вес (грамм)</label>
                <input type="number" name="weight" value="{{ old('weight') }}" class="w-full border rounded p-2" placeholder="Например, 250">
            </div>

            {{-- Количество на складе --}}
            <div>
                <label class="block mb-1 font-medium">Количество на складе</label>
                <input type="number" name="stock" value="{{ old('stock') }}" class="w-full border rounded p-2" placeholder="Оставьте пустым для неограниченного">
                <div class="mt-1 flex items-center space-x-2">
                    <input type="checkbox" name="unlimited" id="unlimited" value="1" {{ old('unlimited') ? 'checked' : '' }} class="rounded">
                    <label for="unlimited" class="text-sm">Всегда в наличии (неограниченно)</label>
                </div>
                <div class="mt-1 flex items-center space-x-2">
                    <input type="checkbox" name="made_to_order" id="made_to_order" value="1" {{ old('made_to_order') ? 'checked' : '' }} class="rounded">
        ...
        <div>
            ...
            <input type="text" name="cooking_technology" value="{{ old('cooking_technology') }}" class="w-full border rounded p-2">
        </div>
