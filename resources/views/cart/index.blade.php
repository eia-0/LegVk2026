...
@section('content')
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Корзина</h1>
    ...
    @else
        ...
        <div class="hidden md:block bg-white rounded-2xl shadow overflow-hidden">
            <table class="w-full">
                <thead class="bg-gray-50 border-b text-sm text-gray-600">
                    <tr>
                        <th class="p-4 text-left font-medium">Товар</th>
                        <th class="p-4 text-center font-medium">Цена</th>
                        <th class="p-4 text-center font-medium">Кол-во</th>
                        <th class="p-4 text-center font-medium">Сумма</th>
                        <th class="p-4"></th>
                    </tr>
                </thead>
                    ...
                    <tr class="border-b hover:bg-gray-50 transition" data-id="{{ $item->id }}">
                        ...
                        <td class="p-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                <button onclick="changeQuantity({{ $item->product_id }}, -1)" class="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600">−</button>
                                <span class="w-6 text-center text-gray-800">{{ $item->quantity }}</span>
                                <button onclick="changeQuantity({{ $item->product_id }}, 1)" class="w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-gray-600">+</button>
                            </div>
                        </td>
                        <td class="p-4 text-center text-amber-600 font-medium">{{ $item->product->price * $item->quantity }} ₽</td>
                        <td class="p-4 text-center">
            ...
            <div class="bg-white rounded-xl shadow p-4 flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center space-x-2 mt-1">
                        ...
                        <span class="w-5 text-center text-gray-800">{{ $item->quantity }}</span>
                        <button onclick="changeQuantity({{ $item->product_id }}, 1)" class="w-7 h-7 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center text-sm text-gray-600">+</button>
                    </div>
                </div>
                <div class="text-right">
                    <p class="text-amber-600 font-medium">{{ $item->product->price * $item->quantity }} ₽</p>
        ...
        <div class="mt-6 bg-white rounded-2xl shadow p-5 flex flex-col sm:flex-row justify-between items-center">
            <span class="text-lg text-gray-700">
                Итого: <span class="text-amber-600 font-medium">{{ number_format($cartTotal, 2) }} ₽</span>
            </span>
            @if($minOrder !== null && $cartTotal < $minOrder)
                <span class="text-gray-500 text-sm mt-2 sm:mt-0">Минимальная сумма не достигнута</span>
            @else
                <a href="{{ route('order.create') }}" class="mt-2 sm:mt-0 bg-amber-500 hover:bg-amber-600 text-white font-medium py-2.5 px-6 rounded-full transition shadow-md">
                    Оформить заказ
                </a>
            @endif
        </div>
